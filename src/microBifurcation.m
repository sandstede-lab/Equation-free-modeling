...
if loadFiles
    ref_1 = readmatrix('../data/ref_108.csv');
    ref_2 = readmatrix('../data/ref_1078.csv');
else
    ...
    ref_2 = allTime_2(end,:)';
    writematrix(ref_1, '../data/ref_108.csv');
    writematrix(ref_2, '../data/ref_1078.csv');
end
...
% plot the bifurcation diagram as it's made
figure; hold on; 
...
writematrix(bif, '../results/microBif.csv')
...
% initGuess - first guess (point on tangent vector)
    function [fw,J] = FW(var, ref, nCars, len, tau, W, initGuess)
        ...
        [fw(1:nCars*discreteScaling+2), J1] = microFJ(var(1:end-1), ref, nCars,...
            discreteScaling*nCars , len, v0, tau, h);
